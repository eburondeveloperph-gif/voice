...
export type IntegrationProviderDefinition = {
  ...
  deprecated?: boolean;
  eburonProvider?: string;
  fields: IntegrationFieldDefinition[];
  buildEburonCredentialPayload?: (args: BuildPayloadArgs) => Record<string, unknown>;
};
  ...
  return Object.entries(integrationCategoryLabels).map(([key, label]) => {
    ...
    const categoryProviders = providers
      ...
      .map((provider) => ({
        ...
        eburonProvider: provider.eburonProvider,
...
export function buildEburonCredentialPayload(
  provider: IntegrationProviderDefinition,
  name: string,
  normalizedConfig: Record<string, unknown>,
): Record<string, unknown> {
  if (!provider.buildEburonCredentialPayload) {
    throw badRequest(`${provider.label} does not map to an Eburon credential payload.`);
  }
  ...
  return provider.buildEburonCredentialPayload({
    name: safeName,
